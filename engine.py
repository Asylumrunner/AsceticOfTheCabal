...
class Engine():
    ...
    def initialize_player(self):
        ...
        libtcod.white, "Player", True, RenderOrder.ACTOR, Fighter(hp=30, defense=2, power=5), None, self.message_log)

    def cull_dead(self):
        # Finds every entity in the game world with 0 health and kills them
        # Returns true if it kills the player, otherwise false
        player_killed = False
        dead_entities = [entity for entity in self.entities if not entity.fighter.isAlive() and entity.char != '%']
    ...
    def main(self):
        ...
        while self.game_running:
            ...
            render_all(self.con, self.panel, self.entities, self.player, self.game_map, self.fov_map, self.fov_recompute, self.message_log, self.mouse)
